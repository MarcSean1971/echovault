...
export function MessageDeliverySettings({
  ...
  return <div className={`space-y-1 ${showInTabs ? 'pt-0' : 'pt-2'}`}>
      <div className="grid grid-cols-1 gap-1 text-sm">
        <div className="grid grid-cols-3 gap-1">
          <span className="font-medium">Type:</span>
          <span className="col-span-2 text-right">{renderConditionType()}</span>
        </div>
        ...
        {condition.condition_type === 'no_check_in' && <>
            ...
            {condition.reminder_hours && condition.reminder_hours.length > 0 && <div className="grid grid-cols-3 gap-1">
                <span className="font-medium">Reminder settings:</span>
                <span className="col-span-2 text-right">
                  {condition.reminder_hours.sort((a: number, b: number) => b - a).map((minutes: number) => formatReminderTime(minutes)).join(', ')}
                </span>
              </div>}
          </>}

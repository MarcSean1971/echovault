...
export function RecipientsListSection({
  ...
  return <Card className="overflow-hidden border border-border/50 shadow-sm">
      <CardContent className="p-6 py-0">
        {renderSectionHeader(<Users className={`h-5 w-5 text-muted-foreground ${HOVER_TRANSITION}`} />, "Recipients")}
        {recipients && recipients.length > 0 ? <RecipientsSection recipients={recipients} isArmed={isArmed} isActionLoading={isActionLoading} onSendTestMessage={onSendTestMessage} /> : <p className="text-muted-foreground text-sm">No recipients</p>}
      </CardContent>
    </Card>;
}
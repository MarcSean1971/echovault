...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    try {
      ...
      console.error(`[AccessFile] Failed to create Supabase client: ${clientError.message}`);
      throw new Error(`Database initialization failed: ${clientError.message}`);
    }
    ...
    if (pathParts.length >= 2) {
      ...
      if (filePathIndex > 0 && filePathIndex < pathParts.length) {
        ...
        while (retryCount < MAX_RETRIES) {
          try {
            console.log(`[AccessFile] Starting security validation (attempt ${retryCount + 1}/${MAX_RETRIES})`);
            
            // Test Supabase connection with a simple query first
            let connectionTest;
            try {
              connectionTest = await supabase
                .from('delivered_messages')
                .select('count(*)', { count: 'exact', head: true });
              
              console.log(`[AccessFile] Connection test: ${connectionTest.error ? 'Failed' : 'Successful'}`);
              
              if (connectionTest.error) {
                console.error(`[AccessFile] Connection test error: ${connectionTest.error.message}`);
                console.error(`[AccessFile] Error code: ${connectionTest.error.code}`);
                console.error(`[AccessFile] Error details: ${JSON.stringify(connectionTest.error.details || {})}`);
                
                // If this is not the last retry, create a new client and try again
                if (retryCount < MAX_RETRIES - 1) {
                  console.log(`[AccessFile] Retrying with new client (${retryCount + 1}/${MAX_RETRIES})`);
                  supabase = createSupabaseClient();
                  retryCount++;
                  continue;
                }
              ...
              console.error(`[AccessFile] Connection error: ${connError.message}`);
              // If this is not the last retry, create a new client and try again
              if (retryCount < MAX_RETRIES - 1) {
                console.log(`[AccessFile] Retrying with new client (${retryCount + 1}/${MAX_RETRIES})`);
                supabase = createSupabaseClient();
            ...
            try {
              ...
              if (fileError || !fileData) {
                // If primary bucket fails, try alternative bucket
                const altBucketName = bucketName === "message-attachments" ? "message_attachments" : "message-attachments";
                console.log(`[AccessFile] Primary bucket failed, trying alternate bucket: ${altBucketName}`);
                ...
                if (altResult.error || !altResult.data) {
                  console.error(`[AccessFile] All bucket attempts failed. Primary error: ${fileError?.message}`);
                  console.error(`[AccessFile] Alternate error: ${altResult.error?.message}`);
                  throw new Error("File not found in any storage bucket");
                }
                
                // Use data from alternative bucket
                return prepareFileResponse(altResult.data, fileType, fileName, downloadMode);
              }
              
              // Use data from primary bucket
            ...
            if (retryCount === MAX_RETRIES - 1) {
              console.error(`[AccessFile] Validation error after ${MAX_RETRIES} attempts: ${validationError.message}`, validationError);
            ...
            // Otherwise, retry
            console.log(`[AccessFile] Validation error, retrying (${retryCount + 1}/${MAX_RETRIES}): ${validationError.message}`);
            
            // Create a new client for the next attempt
            try {
              supabase = createSupabaseClient();
            } catch (clientError) {
              console.error(`[AccessFile] Failed to recreate client: ${clientError.message}`);
            }

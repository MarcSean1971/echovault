 ...
 * CRITICAL FIX: Use the same performUserCheckIn logic as manual check-in
 */
serve(async (req) => {
  ...
  try {
    ...
    const checkInResult = await performUserCheckInInFunction(supabase, userId);
    
    if (!checkInResult) {
      console.error("[CHECK-IN] Failed to update user conditions for WhatsApp check-in");
      return createErrorResponse({ message: "Failed to update user conditions" }, 500);
    }
    ...
    console.log(`[CHECK-IN] FINAL STATUS: Conditions remain ACTIVE, only last_checked updated for ${checkInResult.conditionsUpdated} conditions`);
 ...
 */
async function performUserCheckInInFunction(supabase: any, userId: string): Promise<{ conditionsUpdated: number } | null> {
  try {
    ...
    try {
      ...
      const { error: triggerError } = await supabase.functions.invoke("send-reminder-emails", {
        body: {
          debug: true,
          forceSend: false,
          source: "whatsapp-check-in-function-trigger",
          action: "process"
        }
      });
